...
use clawhive_domain::policy::{PolicyEffect, PolicyRule};
...
impl IcvsCompiler {
    ...
    pub fn compile_policy(source: &str) -> Result<Vec<PolicyRule>, IcvsError> {
        ...
        for (id, node) in &doc.nodes {
            ...
            let rule = PolicyRule {
                id: clawhive_domain::policy::PolicyRuleId(uuid::Uuid::new_v7(uuid::Timestamp::now(uuid::NoContext))),
                subject: clawhive_domain::policy::PolicySubject::Agent("*".to_string()),
                effect,
                action: "*".to_string(),
