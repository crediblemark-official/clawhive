...
//!
//! Config file discovery: `./clawhive.toml`, `~/.config/clawhive/config.toml`,
//! or `$CLAWHIVE_CONFIG`.
//!
//! # Format
//! ```toml
...
pub fn discover_config() -> Option<ClawHiveConfig> {
...
fn config_file_candidates() -> Vec<PathBuf> {
    ...
    // 1. Env var override
    if let Ok(path) = std::env::var("CLAWHIVE_CONFIG") {
        candidates.push(PathBuf::from(path));
    }

    // 2. Current working directory
    if let Ok(cwd) = std::env::current_dir() {
        candidates.push(cwd.join("clawhive.toml"));
    }

    // 3. XDG config directory (~/.config/clawhive/config.toml)
    if let Some(config_dir) = dirs::config_dir() {
        candidates.push(config_dir.join("clawhive").join("config.toml"));
    }

    // 4. Home directory (~/.clawhive.toml)
    if let Some(home) = dirs::home_dir() {
        candidates.push(home.join(".clawhive.toml"));
    }
