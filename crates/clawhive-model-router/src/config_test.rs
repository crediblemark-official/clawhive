...
"#;
    let config: ClawHiveConfig = toml::from_str(toml_str).expect("should parse");
...
fn test_config_discovery_candidates() {
    ...
    // First candidate should be from CLAWHIVE_CONFIG env var if set,
    // otherwise from cwd
    assert!(candidates.iter().any(|p| p.ends_with("clawhive.toml")));
}
...
"#;
    let config: ClawHiveConfig = toml::from_str(config_toml).unwrap();
    let kv = |_: &str| None;
...
fn test_resolve_providers_bare_slot() {
    ...
    let config: Option<ClawHiveConfig> = None;
    let kv = |_: &str| None;
