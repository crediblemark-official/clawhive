...
use claw10_domain::{Agent, CheckpointReason, RuntimeLease};
use claw10_event::Claw10Event;
use claw10_event::events::{WakeTrigger};
...
pub async fn hibernate_agent(
    ...
    let _ = state.event_bus.publish(Claw10Event::AgentHibernated {
        agent_id: agent.id.0,
...
pub async fn wake_agent(
    ...
    let _ = state.event_bus.publish(Claw10Event::AgentWoken {
        agent_id: agent.id.0,
...
pub async fn migrate_agent(
    ...
    let _ = state.event_bus.publish(Claw10Event::AgentMigrated {
        agent_id: agent.id.0,
        from_worker,
