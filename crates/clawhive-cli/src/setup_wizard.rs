...
impl SetupWizard {
    ...
    fn load_static_models(&self) -> Vec<String> {
        use clawhive_model_router::models;
    ...
    fn save_config(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !self.api_key.is_empty() {
            ...
            let env_dir = std::path::PathBuf::from(&home).join(".clawhive");
    ...
    fn draw_welcome(&self, frame: &mut Frame, area: Rect) {
        let lines = vec![
            Line::from(""),
            Line::from(vec![
                Span::styled("  Selamat datang di ClawHive OS!", Style::default().fg(Color::Rgb(254, 192, 126)).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
            Line::from(vec![
    ...
    fn draw_complete(&self, frame: &mut Frame, area: Rect) {
        let lines = vec![
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled("    clawhive tui", Style::default().fg(Color::Rgb(254, 192, 126)).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(vec![
                Span::styled("    clawhive serve", Style::default().fg(Color::Rgb(254, 192, 126)).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
            Line::from(vec![
